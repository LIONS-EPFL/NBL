...
class MistralDecoderLayer(nn.Module):
    def __init__(self, config: MistralConfig, layer_idx: int):
        ...
        if self.drop_attn:
            ...
            with open("./llm_variables/xlayer_"+str(layer_idx)+"_weights.pkl", "rb") as f:  # Python 3: open(..., 'rb')
                weight, bias = pickle.load(f)
            #self.lin = nn.Linear(weight.size(1), weight.size(0))
            dtype = next(self.parameters(), torch.tensor(1.0)).dtype

            self.register_buffer("weight", weight.to(dtype))
            self.register_buffer("bias", bias.to(dtype).squeeze(1))

            #self.lin.weight.data.copy_(torch.eye(weight.size(1)).to(dtype))
            #self.lin.bias.data.copy_(bias.to(dtype).squeeze(1)*0)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if not self.drop_attn:
            ...
            #hidden_states = self.lin(hidden_states)
            residual = hidden_states
            hidden_states = torch.matmul(hidden_states, self.weight.T) + self.bias
            hidden_states = residual + hidden_states
        ...
        if output_attentions and not self.drop_attn:
